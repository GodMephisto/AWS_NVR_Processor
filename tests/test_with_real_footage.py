...
class RealFootageTester:
    ...
    def create_realistic_test_videos(self):
        ...
        logger.info(f"✅ Created {len(self.sample_videos)} realistic video files")
    ...
    def test_metadata_extraction_real_files(self):
        ...
        try:
            ...
            for video_info in self.sample_videos[:5]:  # Test first 5 files
                ...
                try:
                    ...
                    if metadata:
                        ...
                        logger.warning(f"   ⚠️  No metadata extracted from {Path(filepath).name}")
            ...
            logger.error(f"❌ Cannot import metadata extractor: {e}")
            return False
        except Exception as e:
    ...
    def test_cloud_sync_real_files(self):
        ...
        logger.info("\n☁️  Testing Cloud Sync with Real-like Files...")
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info("✅ Cloud sync dry-run successful")
                ...
                if files_found >= len(self.sample_videos) * 0.8:  # 80% detection rate
                    logger.info("   ✅ File detection rate acceptable")
                    return True
                else:
                    logger.warning(f"   ⚠️  Low file detection rate: {files_found}/{len(self.sample_videos)}")
                    return False
            else:
            ...
            logger.error(f"❌ Cloud sync test error: {e}")
            return False
    ...
    def test_file_monitoring(self):
        ...
        try:
            ...
            if new_filepath.exists() and new_filepath.stat().st_size > 0:
                logger.info("   ✅ New file created and accessible")
                ...
                try:
                    ...
                    if metadata:
                        logger.info("   ✅ New file metadata extracted successfully")
                        return True
                    else:
                        logger.warning("   ⚠️  Could not extract metadata from new file")
                        return False
                        
                except Exception as e:
                    logger.error(f"   ❌ New file processing failed: {e}")
                    return False
            else:
                logger.error("   ❌ New file not accessible")
                return False
    ...
    def test_vod_with_real_files(self):
        ...
        try:
            ...
            if vod_process.poll() is None:
                logger.info("   ✅ VOD server started")
                ...
                try:
                    ...
                    if response.status_code == 200:
                        logger.info("   ✅ VOD API health check passed")
                        ...
                        if search_response.status_code == 200:
                            logger.info("   ✅ VOD video search working")
                            ...
                            if data.get('videos'):
                                ...
                                if stream_response.status_code == 200:
                                    logger.info("   ✅ VOD streaming URL generation working")
                                    vod_process.terminate()
                                    return True
                    
                    logger.error("   ❌ VOD API tests failed")
                    vod_process.terminate()
                    return False
                    
                except requests.exceptions.RequestException as e:
                    logger.error(f"   ❌ VOD API connection failed: {e}")
                    vod_process.terminate()
                    return False
            else:
